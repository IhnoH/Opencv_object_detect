...
class Detect_object:
    ...
    def a4_init(self, src):
        ...
        for i, dot in enumerate(approx):
            ...
            # print(x, y, self.a4_list[i])    # result 해상도dsa

            for p in cordi[i]: cv.circle(src_, (p[0], p[1]), 2, (0, 255, 255), -1)  # 검은색 점

            #print(cordi[i][0][0], cordi[i][0][1], x, y)# , round(self.shape_rate[i], 3), round(self.a4_rate[i], 3), round(self.a4_list[i], 3))
        #print('-----')

        # 검은색 점이 a4로 잡힘 문제가 뭔지 몰라 좌표에 계산된 것을 표시해봐야함
        ...
        for i, dot in enumerate(cordi):
            ...
            if len(abs_distance) == 4:
                ...
                #print(r1, r2, (np.mean([r1, r2])))
                self.a4_rate.append([r1 - 0.70707070707, r2 - 0.70707070707, np.mean([r1, r2]) - 0.70707070707])
                #rate.append(self.a4_rate[-1] + self.shape_rate[-1])


                try: x, y = np.shape(res[i])[0], np.shape(res[i])[1]
                except:
                    # print('except')
                    # print(res[i])
                    return
            ...
            cv.putText(src_, '{0} {1} {2} {3} {4}'.format(x, y, round(self.shape_rate[i], 3), round(min(self.a4_rate[i]), 3),
    ...
    def object_sizing(self, src):
        ...
        for i in range(len(del_list)-1, -1, -1):
            ...
            del abs_distance[i]
        '''
        #print('a4_rate', self.a4_rate)
        #print('a4_list', self.a4_list)
        for i, cordi in enumerate(approx_):
            ...
            c = round(abs_distance[i][0][2] * (self.px_per_mm[0] + self.px_per_mm[1])/2, 2)
            ...
            cv.putText(src, '{} {} {}'.format(a, b, (a + b) / 2),
                       (int(cordi[0][0] + (abs_distance[i][0][0] / 2) + 5),
            ...
            #print(a, b, c, (a + b) / 2, abs_distance[i][0][2])

            #print(min(a, b)*(self.a4_rate[0][2]+1), ((a + b) / 2)*(self.a4_rate[0][2]+1))
            for xy in cordi:
                cv.circle(src, tuple(xy), 1, (255, 0, 255), 1)


        #print(self.px_per_mm, min(self.px_per_mm[0], self.px_per_mm[1]) / max(self.px_per_mm[0], self.px_per_mm[1]))
    ...
    def del_similar(self, approx):
        ...
        for i in range(len(diff_approx) - 1):
            ...
            for j in range(len(diff) - 1):
                dots.append(np.abs(diff[j][0] - diff[j + 1][0]))

            if len(dots) == 1:
                if math.sqrt(dots[0][0] ** 2 + dots[0][1] ** 2) < 100: dlt = True
                else: dlt = False

            for j in range(len(dots)-1):
                d = np.abs(dots[j] - dots[j + 1])
                #print(i, math.sqrt(d[0] ** 2 + d[1] ** 2))

                if math.sqrt(d[0] ** 2 + d[1] ** 2) < 10: dlt = True
                else: dlt = False
